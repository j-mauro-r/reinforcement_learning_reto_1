...
def export_inference_model(
    ...
    Args:
        checkpoint_path: Full checkpoint generated by ``CheckpointManager``.
        output_path: Destination ``.pt`` path for the compact artifact.
        project_run_id: Logical run identifier for lineage.
        config: Optional expected configuration. When omitted, the checkpoint
            embedded configuration is used.
        source_checkpoint_step: Optional explicit checkpoint step. When omitted,
            the value is read from the checkpoint.
        repo_path: Repository path used to record the current Git SHA.
        extra_metadata: Additional JSON-serializable metadata.
        overwrite: Whether an existing artifact may be replaced.
        max_size_bytes: Guardrail for compact artifact size.

    Returns:
        Export metadata including SHA-256 and size.

    Raises:
        FileNotFoundError: If the source checkpoint does not exist.
        FileExistsError: If the output exists and overwrite is false.
        ValueError: If the checkpoint/config is incompatible.
    """
...
def load_inference_model(
    ...
    Args:
        artifact_path: Path created by ``export_inference_model``.
        device: Optional torch device for the loaded agent.
        expected_sha256: Optional checksum to validate before loading weights.
        expected_project_run_id: Optional lineage guardrail.

    Returns:
        A new ``DDQNAgent`` and artifact metadata.

    Raises:
        FileNotFoundError: If the artifact does not exist.
        ValueError: If schema, metadata, checksum or weights are invalid.
    """
...
def _build_metadata(
    ...
    if extra_metadata:
        metadata.update({key: value for key, value in extra_metadata.items() if key != "config_fingerprint"})
    return metadata
